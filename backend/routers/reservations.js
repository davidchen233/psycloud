...
router.get(
  "/psychologistEvents/unreserved/:psychologistId",
  async (req, res) => {
    let data = await connection.queryAsync(
...
router.get("/getUser/getList/", async (req, res) => {
...
router.get("/getUser/getPsyList/", async (req, res) => {
  const id = req.session.user.id;
  console.log(id);
  const sql =
    "SELECT psy.id from psychologists AS psy INNER JOIN users on users.id = psy.user_id WHERE user_id = ?";
  let psyId = await connection.queryAsync(sql, [id]);
  const sql2 =
  ...
  let data = await connection.queryAsync(sql2, [psyId[0].id]);
  res.json(data);
});
